...
4. Compara las imágenes de consulta con el dataset usando distancias
   Euclidiana y Coseno.
5. Muestra los resultados: la imagen de consulta y sus vecinos más cercanos.
"""
...
# Asume la siguiente estructura de carpetas:
# ./search.py
# ./data/dataset/ (imágenes del dataset)
# ./data/query/ (imágenes de consulta)
# ./features/ (se creará para guardar el caché .npy)
...
# --- Modelo Global ---
# Se define globalmente para que las funciones de extracción puedan usarlo.
try:
    ...
    exit() # Salir si el modelo no puede cargarse
...
def cargar_imagenes(carpeta):
    ...
    rutas = []
    # Busca los tipos de imagen más comunes
...
def extract_features(image_path):
    ...
    # verbose=0 evita que imprima un log por cada predicción
...
def buscar_vecino_mas_cercano(features_dataset, features_query, nombres_dataset):
    ...
    for i, f_query in enumerate(features_query):
        ...
        # Encontrar el índice (vecino) con la menor distancia
...
def visualizar_resultados(resultados, imagenes_query_rutas, nombres_query):
    ...
    for r in resultados:
        ...
        try:
            ...
            img_euc = image.load_img(os.path.join(DATASET_PATH, r['vecino_euc']))
            img_cos = image.load_img(os.path.join(DATASET_PATH, r['vecino_cos']))
        except FileNotFoundError as e:
            print(f"Advertencia: No se pudo cargar una imagen para visualización: {e}")
            continue
...
def main():
    # 1. Asegurar que exista el directorio de características
    ...
    features_dataset = np.empty((0, 2048)) # ResNet50 'avg' pooling da 2048 dims
    ...
    # Usar un set para búsqueda rápida (O(1)) de imágenes ya procesadas
    ...
    for i, img_path in enumerate(imagenes_dataset_rutas):
        ...
        # Si la imagen actual no está en el set de cacheados, procesarla
    ...
    # Unir características viejas y nuevas
    if new_features:
        ...
        # Guardar las características y nombres actualizados en caché
        try:
    ...
    # 4. Extracción de características de consulta (estas no se cachean)
    ...
    # 6. Visualización
    visualizar_resultados(resultados, imagenes_query_rutas, nombres_query)
